...
void PclRecorderROS2::callback(const sensor_msgs::msg::PointCloud2::SharedPtr cloud)
{
  ...
  try {
    transform = tf2::transformToEigen (tf_buffer_->lookupTransform(fixed_frame_, cloud->header.frame_id,  cloud->header.stamp, rclcpp::Duration::from_nanoseconds(1000000000)));
